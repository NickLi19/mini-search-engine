...
for word in words_matrix.keys():
    word_number += 1
    df_dict[word_number] = document_number - words_matrix[word].count(0)

words_expansion = {'beautiful': ['nice', 'fancy'], 'chapter': ['chpt'], 'chpt': ['chapter'],
                    'responsible': ['owner', 'accountable'], 'freemanmoore': ['freeman', 'moore'],
                    'dept': ['department'], 'photo': ['photograph', 'image', 'picture'],
                    'brown': ['beige', 'tan', 'auburn'], 'tues': ['Tuesday'],
                    'sole': ['owner', 'single', 'shoe', 'boot'], 'homework': ['hmwk', 'home', 'work'],
    ...
    if word_flag:
        ...
        for i in range(1, document_number+1):
            ...
            for idx, tf in enumerate(documents_list[0]):
                if tf > 0:
                    tf_temp = (1 + math.log(tf)) * math.log(document_number/df_dict[idx+1])
                else:
                    tf_temp = 0
                query_sum += tf_temp * tf_temp
                query_score += [tf_temp]
            query_normalized = math.sqrt(query_sum)
            query_score_processed = np.array([score/query_normalized for score in query_score])
            ...
            document_score_processed = np.array([score/document_normalized for score in document_score])
        ...
        if not is_expanded:
            if result_count < 5:
                print('---------------------------')
                print('We did not find enought articles about this query. Now starts query expansion:')
                is_expanded = True
        ...
        if is_expanded:
            ...
            print('---------------------------')
            print('We did not find article about this query. Now let us try query expansion:')
            is_expanded = True
            last_query_list = [query_word for query_word in query_list]
